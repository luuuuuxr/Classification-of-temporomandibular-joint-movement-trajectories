...
class FinalTrajectory3DFeatureExtractor:
    """最终版3D轨迹特征提取器"""
    ...
    def extract_comprehensive_3d_features(self, trajectory):
        ...
        for coord in range(3):  # x, y, z
            ...
            features.extend([
                ...
                stats.skew(coord_data),   # 偏度
                stats.kurtosis(coord_data),  # 峰度
            ])
...
def load_realistic_challenging_data():
    """生成更真实且具有挑战性的3D轨迹数据"""
    print("生成具有挑战性的3D轨迹数据...")
    
    np.random.seed(42)
    n_samples = 900  # 增加样本数量
    time_steps = 200
...
def compare_approaches_final(X, y, save_path="results"):
    """最终的对比实验"""
    # 确保保存路径存在
    ...
    # 提取3D特征
    print("\n提取高级3D特征...")
    extractor = FinalTrajectory3DFeatureExtractor()
    ...
    plt.tight_layout()
    plt.savefig(f"{save_path}/final_confusion_matrices.png", dpi=300, bbox_inches="tight")
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f"{save_path}/final_feature_distributions.png", dpi=300, bbox_inches="tight")
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f"{save_path}/final_3d_trajectory_visualization.png", dpi=300, bbox_inches="tight")
    plt.show()
    
    # 保存详细结果
    print("\n保存详细结果...")
    
    with open(f"{save_path}/final_comparison_results.txt", "w", encoding="utf-8") as f:
        f.write("最终3D轨迹架构对比结果\n")
        f.write("=" * 60 + "\n\n")
        ...
        f.write(f"\n5. 结论:\n")
        best_raw_acc = max([results[name]["val_accuracy"] for name in raw_models])
        best_feat_acc = max([results[name]["val_accuracy"] for name in feat_models])
        if best_feat_acc > best_raw_acc:
            f.write(f"   3D特征工程在验证集上提升了 {best_feat_acc - best_raw_acc:.4f} 的准确率\n")
        elif best_feat_acc < best_raw_acc:
            f.write(f"   原始数据在验证集上比3D特征工程高 {best_raw_acc - best_feat_acc:.4f} 的准确率\n")
        else:
            f.write(f"   3D特征工程和原始数据在验证集上表现相同\n")
        
        f.write(f"   3D特征工程将特征维度从 {X_train_flat.shape[1]} 压缩到 {features_train.shape[1]}\n")
        f.write(f"   维度压缩比: {X_train_flat.shape[1] / features_train.shape[1]:.2f}\n")
    
    # 保存分类报告
    with open(f"{save_path}/final_classification_reports.txt", "w", encoding="utf-8") as f:
        f.write("最终分类报告\n")
        f.write("=" * 60 + "\n\n")
    ...
    print(f"包含文件:")
    print(f"  - final_performance_comparison.png: 最终性能比较图")
    print(f"  - final_confusion_matrices.png: 最终混淆矩阵比较图")
    print(f"  - final_feature_distributions.png: 最终特征分布图")
    print(f"  - final_3d_trajectory_visualization.png: 最终3D轨迹可视化图")
    print(f"  - final_comparison_results.txt: 最终性能结果")
    print(f"  - final_classification_reports.txt: 最终分类报告")
...
def main():
    ...
    # 生成数据
    X, y = load_realistic_challenging_data()
    print(f"数据形状: {X.shape}")
    print(f"标签分布: {np.bincount(y)}")
    
    # 运行对比实验
    results = compare_approaches_final(X, y)
